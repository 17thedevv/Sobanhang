...
class StockCheckController {
    async getChecks(req, res) {
        try {
            ...
            if (status)
                where.status = status;
            const checks = await prisma_1.prisma.stockCheck.findMany({
                where,
    ...
    async getCheckById(req, res) {
        try {
            const id = req.params.id;
            const storeId = req.user?.storeId;
            const check = await prisma_1.prisma.stockCheck.findFirst({
                where: { id, storeId },
                include: {
                    items: {
                        include: {
            ...
            if (!check)
                return res.status(404).json({ message: 'Stock check not found' });
            res.json({ check });
        }
        catch (error) {
    ...
    async createCheck(req, res) {
        try {
            ...
            if (!storeId)
                return res.status(403).json({ message: 'Store required' });
            const { items, // array of { productId, systemQuantity, actualQuantity, diffQuantity }
            status, // CHECKING or BALANCED
            note } = req.body;
            const code = generateCheckCode();
            const check = await prisma_1.prisma.$transaction(async (tx) => {
                // 1. Create Check
        ...
        try {
            ...
            const check = await prisma_1.prisma.$transaction(async (tx) => {
                ...
                if (existing.status !== 'CHECKING')
                    throw new Error('Can only update CHECKING status');
