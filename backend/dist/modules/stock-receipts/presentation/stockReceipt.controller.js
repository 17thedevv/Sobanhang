...
exports.StockReceiptController = void 0;
...
class StockReceiptController {
    async getReceipts(req, res) {
        try {
            ...
            const receipts = await prisma_1.prisma.stockReceipt.findMany({
                where,
    ...
    async getReceiptByCode(req, res) {
        try {
            const code = req.params.code;
            const storeId = req.user?.storeId;
            const receipt = await prisma_1.prisma.stockReceipt.findFirst({
                where: { code, storeId },
                include: {
                    supplier: true,
                    items: {
    ...
    async createReceipt(req, res) {
        try {
            ...
            if (!storeId)
                return res.status(403).json({ message: 'Store required' });
            const { supplierId, items, // array of { productId, quantity, importPrice, subTotal }
            totalAmount, discount, shippingFee, finalAmount, paidAmount, status, // DRAFT or COMPLETED
            note, cashSourceId } = req.body;
            const code = generateReceiptCode();
            const receipt = await prisma_1.prisma.$transaction(async (tx) => {
                // 1. Create Receipt
    ...
    async confirmReceipt(req, res) {
        try {
            ...
            if (receipt.status !== 'DRAFT')
                return res.status(400).json({ message: 'Receipt is not in DRAFT status' });
    ...
    async deleteReceipt(req, res) {
        try {
            ...
            await prisma_1.prisma.stockReceipt.delete({
                where: { id }
            });
...
exports.StockReceiptController = StockReceiptController;
