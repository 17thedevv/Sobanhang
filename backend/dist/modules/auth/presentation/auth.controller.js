...
class AuthController {
    async registerEmail(req, res) {
        try {
            ...
            const result = await auth_service_1.authService.registerEmail(email);
    ...
    async verifyOtp(req, res) {
        try {
            ...
            const setupToken = jsonwebtoken_1.default.sign({ userId: result.userId, scope: 'setup' }, JWT_SECRET, { expiresIn: '1h' });
    ...
    async setPassword(req, res) {
        try {
            ...
            const user = await auth_service_1.authService.setPassword(userId, password);
            const JWT_SECRET = process.env.JWT_SECRET || 'super_secret_key_sobanhang';
            const accessToken = jsonwebtoken_1.default.sign({ userId: user.userId, role: user.role, scope: 'access', storeId: user.storeId }, JWT_SECRET, { expiresIn: '7d' });
            // Xóa setupToken cũ
    ...
    async login(req, res) {
        try {
            ...
            const JWT_SECRET = process.env.JWT_SECRET || 'super_secret_key_sobanhang';
            const accessToken = jsonwebtoken_1.default.sign({ userId: user.userId, role: user.role, scope: 'access', storeId: user.storeId }, JWT_SECRET, { expiresIn: rememberMe ? '30d' : '1d' } // Nếu không nhớ, chỉ lưu 1 ngày
            );
            const cookieOptions = {
                httpOnly: true,
                secure: true,
    ...
    async googleLogin(req, res) {
        try {
            ...
            const user = await auth_service_1.authService.loginWithGoogle(idToken);
            const JWT_SECRET = process.env.JWT_SECRET || 'super_secret_key_sobanhang';
            ...
            if (user.status === 'ACTIVE') {
                ...
                const setupToken = jsonwebtoken_1.default.sign({ userId: user.userId, role: user.role, status: user.status }, JWT_SECRET, { expiresIn: '1h' });
                res.cookie('setupToken', setupToken, {
                    httpOnly: true,
                    secure: process.env.NODE_ENV === 'production',
                    sameSite: process.env.NODE_ENV === 'production' ? 'none' : 'lax',
    ...
    async verifyResetOtp(req, res) {
        try {
            ...
            const result = await auth_service_1.authService.verifyForgotPasswordOtp(email, otp);
            const JWT_SECRET = process.env.JWT_SECRET || 'super_secret_key_sobanhang';
            // Cấp resetToken sống trong 15 phút
            const resetToken = jsonwebtoken_1.default.sign({ userId: result.userId }, JWT_SECRET, { expiresIn: '15m' });
    ...
    async resetPassword(req, res) {
        try {
            ...
            await auth_service_1.authService.resetPassword(userId, newPassword);
